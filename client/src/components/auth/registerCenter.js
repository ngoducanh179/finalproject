...
const RegisterCustomer = ({ setAlert, registerCustomer, isAuthenticated, auth }) => {
    ...
    const onChange = e => {
        ...
        if (e.target.name === 'address') {
            ...
            function error(err) {
                ...
                return setAlert('Hãy Chấp Nhận Quyền Xem Vị Trí Trên Trình Duyệt Của Bạn', 'danger');
            }
    ...
    const confirmOTP = async code => {
        try {
            ...
            if (confirmInfo && confirmInfo.user && confirmInfo.user.phoneNumber) {
                registerCustomer({ name, email, password, location, phone, confirm: true, role: role.CENTER });
            } else {
                return setAlert('Không Thể Xác Nhận Số Điện Thoại', 'danger')
            }
        } catch (error) {
            return setAlert('Không Thể Xác Nhận Số Điện Thoại', 'danger')
        }
    ...
    const onSubmit = async e => {
        ...
        } else if (!(formData && formData.location && formData.location.address)) {
            setAlert('Không Thể Lấy Vị Trí Hiện Tại Của Bạn', 'danger');
        } else {
        ...
        <Fragment>
            {
                !openModal ? (
                    <Fragment>
                        <br />
                        <h1 className='text-primary'>Bắt Đầu Thêm Phòng Gym Của Bạn Vào TomFit</h1>
                        <p className='lead'>
                            <i className='fas fa-user'></i> Tạo Tài Khoản Phòng Gym Của Bạn Để Thêm Vào Tomfit
      </p>
                        <form className='form' onSubmit={e => onSubmit(e)}>
                            ...
                            <div>
                                <input
                                    type='text'
                                    placeholder='Địa Chỉ Phòng Gym Của Bạn'
                                    name='address'
                        ...
                        <p className='my-1'>
                            Already have an account? <Link to='/login/center'>Đăng Nhập</Link>
                        </p>
                    </Fragment>
                ) : (
                        <div>
